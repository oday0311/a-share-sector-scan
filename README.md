...
- 缠论候选更完整：点击搜索框即可展示 `宽基指数 / 行业指数 / 热门股票 / 港股热门` 分组候选，指数筛选优先覆盖主流宽基、科创、红利和行业指数。
...
- 缠论分析支持 A 股、港股、宽基指数和行业指数搜索，点击输入框可直接选择常用指数和热门股票。
- 缠论页面当前只开放日线复盘，前端不展示周线和分钟级周期入口。
...
6. 切换到“缠论分析”页签后，点击搜索框即可看到常用候选，候选按 `宽基指数 / 行业指数 / 热门股票 / 港股热门` 分组。
7. 在缠论搜索下拉中点击“指数”筛选，可快速选择上证、深证、创业板、科创、沪深 300、中证 500/1000、红利指数、中证行业和全指行业等指数。
8. 选择 A 股、港股、宽基指数或行业指数后，页面按所选日期复盘日线结构；当前前端不展示周线和分钟级周期入口。
9. 缠论页点击“重新分析”会重新拉取 K 线、重新计算结构、重新生成摘要，并覆盖同一标的、周期、日期的缓存。
...
GET /api/chanlun/search?q=中证消费&market=index
```

参数：

- `q`：搜索关键字，支持名称、A 股代码、港股代码和指数代码；为空时返回内置常用候选。
- `market`：可选，`all`、`a`、`hk`、`index`，缺省为 `all`。

返回字段：

- `items`：候选标的列表，包含 `code`、`symbol`、`name`、`market`、`unit`，内置候选还会带有 `group` 分组。
- `meta`：查询关键字、市场过滤条件和结果数量。

当前内置指数候选覆盖两类：

- 宽基指数：上证指数、深证成指、创业板指、沪深 300、中证 500、中证 1000、上证 50、上证 180、上证 380、中证 100、中证 800、中证全指、科创综指、科创 50、深证 100、中小 100、创业板 50、国证 2000、红利指数等。
- 行业指数：中证能源、中证材料、中证工业、中证可选、中证消费、中证医药、中证金融、中证信息、中证电信、中证公用、全指行业系列、银行、证券、军工、新能源、新能源车、医疗、传媒、煤炭、有色、环保等。
...
GET /api/chanlun/analyze?symbol=hk00700&period=day&date=2026-06-12&refresh=1
```

参数：

- `symbol`：必填，支持 `600519`、`sh600519`、`hk00700`、`sh000001` 等形式。
- `period`：可选，页面当前使用 `day`；后端保留 `week` 兼容能力，第一版不实现分钟级周期。
...
9. 打开 `http://127.0.0.1:8765/chanlun`，搜索 `腾讯`、`600519` 或 `中证消费`，确认日线能返回真实 K 线和结构分析。
