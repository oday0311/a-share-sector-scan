...
这是一个面向个人投资者的本地 AI 分析终端：先用全量行业板块数据找出当日强弱主线，再下钻到 A 股、港股和指数的缠论结构，帮助你快速回答三个问题：市场主线在哪里、资金是否延续、关键标的处在什么结构位置。

系统优先使用 WeStock Data / 腾讯自选股行情，AKShare 作为备用数据源；后端规则负责涨跌、资金、趋势和缠论结构识别，AI 只做复盘总结与信号解释，不提供投资建议。
...
- 板块扫描聚焦主线：基于全量标准行业板块复盘，剔除概念、主题、昨日涨停等噪音标签。
- 缠论分析用于下钻：支持 A 股、港股、宽基指数、行业指数的日线结构复盘。
